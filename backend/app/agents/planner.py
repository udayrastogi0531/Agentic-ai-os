...
class PlanStep(BaseModel):
    ...
    tool: str = Field(description="The tool to execute. Must be one of: files, tasks, notes, voice, memory, browser, gmail, calendar, coding, research, github, computer, whatsapp, direct_answer")
    argument: str = Field(description="Parameters or search queries for the tool execution")


class PlannerOutput(BaseModel):
    intent: str = Field(description="User intent classification: files, tasks, notes, voice, memory, browser, gmail, calendar, coding, research, github, computer, whatsapp, direct_answer")
...
12. `computer` - Open local apps (VS Code, Chrome), open files, directories.
13. `whatsapp` - Open WhatsApp Web, find contact, send message, check chats, or schedule follow-up reminders.
14. `direct_answer` - Use when the request is conversational and requires no tools.
